...
async function request(path, options = {}) {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        `${response.status}: ${body.message || "Backend request failed"}`,
      );
    return body;
  } finally {
...
export async function claimJob() {
  const response = await request(
    `/api/print-agent/jobs/claim?wait=${encodeURIComponent(config.longPollWaitMs)}`,
    { method: "POST", body: "{}" },
  );
  return response.data || null;
}
export async function getAgentConfig() {
  return (await request("/api/print-agent/config")).data;
}
