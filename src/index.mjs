...
async function processNextJob() {
  ...
  try {
    ...
    if (Date.now() - printersLoadedAt >= PRINTER_CONFIG_TTL_MS)
      await refreshPrinters();
    const printer = printers.get(String(job.printerId));
    if (!printer)
      throw new Error(`Printer configuration not found for ${job.printerId}`);
    const copies =
      job.kind === "test"
        ? Math.min(50, Math.max(1, Number(job.payload?.copies) || 1))
        : 1;
    ...
    await failJob(job._id || job.id, error).catch((failError) =>
      log("Could not report failed print job", failError),
    );
  }
  return true;
}
...
async function refreshPrinters() {
  ...
  printers = new Map(
    (agentConfig?.printers || []).map((printer) => [
      String(printer._id),
      printer,
    ]),
  );
